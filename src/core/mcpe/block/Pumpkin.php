	...
	public function place(Item $item, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector, Player $player = null) : bool {
		...
		if($player instanceof Player) {
			...
			if(Entity::checkSnowGolemStructure($this)[0]) {
			    $level->setBlock($this, new Block(0));
			    $level->setBlock($this->subtract(0, 1), new Block(0));
			    $level->setBlock($this->subtract(0, 2), new Block(0));
			...
			if($check[0]){
			    switch($check[1]){
			        case "X":
			            $level->setBlock($this->subtract(1, 1, 0), new Block(0));
			            $level->setBlock($this->add(1, -1, 0), new Block(0));
			        break;
					case "Z":
					    $level->setBlock($this->subtract(0, 1, 1), new Block(0));
					    $level->setBlock($this->add(0, -1, 1), new Block(0));
					break;
			    }
			    $level->setBlock($this, new Block(0));
			    $level->setBlock($this->subtract(0, 1), new Block(0));
