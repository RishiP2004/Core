...
class MuteIp extends PluginCommand {
    ...
    public function __construct(Core $core) {
        parent::__construct("mute-ip", $core);
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool {
        ...
        if(count($args) < 2) {
            $sender->sendMessage($this->core->getErrorPrefix() . "Usage: /mute-ip" . " " . $this->getUsage());
            return false;
        }
		$this->core->getStats()->getCoreUser($args[0], function($user) use ($sender, $args) {
			...
			if($muteList->isBanned($ip)) {
				...
				if($player instanceof CorePlayer) {
					$player->sendMessage($this->core->getPrefix() . "You have been Ip-Muted By: " . $sender->getName() . " for the Reason: " . $reason . ". Expires: " . $expire);
				}
				$sender->sendMessage($this->core->getPrefix() . "You have Ip-Muted " . $user->getName() . " for the Reason: " . $reason . ". Expires: " . $expire);
				$this->core->getServer()->broadcastMessage($this->core->getPrefix() . $user->getName() . " has been Ip-Muted by " . $sender->getName() . " for the Reason: "  . $reason . ". Expires: " . $expire);
				return true;
			}
        });
		return false;
