...
class MuteIpCommand extends PluginCommand {
    private $GPCore;

    public function __construct(GPCore $GPCore) {
        parent::__construct("mute-ip", $GPCore);

        $this->GPCore = $GPCore;

        $this->setPermission("GPCore.Essentials.Defaults.Command.MuteIP");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool {
        ...
        if(count($args) < 2) {
            $sender->sendMessage($this->GPCore->getBroadcast()->getErrorPrefix() . "Usage: /mute-ip" . " " . $this->getUsage());
            return false;
        }
        ...
        if(preg_match("/^([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])$/", $args[0])) {
            if($banList->isBanned($user->getUsername())) {
                $sender->sendMessage($this->GPCore->getBroadcast()->getErrorPrefix() . $user->getUsername() . " is already IP Muted");
                return false;
            }
        ...
        foreach($sender->getServer()->getOnlinePlayers() as $onlinePlayers) {
            if($onlinePlayers->getAddress() === $args[0]) {
                $onlinePlayers->sendMessage($this->GPCore->getBroadcast()->getPrefix() . "You have been IP Muted By: " . $sender->getName() . " for the Reason: " . $reason);
            }
        }
        $sender->sendMessage($this->GPCore->getBroadcast()->getPrefix() . "You have IP Muted " . $user->getUsername() . " for the Reason: " . $reason);
        return true;
    }
}
