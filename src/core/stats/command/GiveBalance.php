...
class GiveBalance extends PluginCommand {
	...
	public function __construct(Stats $manager) {
		parent::__construct("givebalance", Core::getInstance());
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args) : bool {
		...
		if(count($args) < 2) {
			$sender->sendMessage(Core::ERROR_PREFIX . "Usage: /givebalance " .  $this->getUsage());
			return false;
		}
		...
		$this->manager->getCoreUser($args[0], function($user) use ($sender, $args) {
			...
			if($user->getBalance() + $args[1] > Statistics::MAXIMUMS["balance"]) {
				...
				if($player instanceof CorePlayer) {
					$player->sendMessage(Core::PREFIX . $sender->getName() . " Gave you " . Statistics::UNITS["balance"] . $args[1]);
				}
				$sender->sendMessage(Core::PREFIX. "Gave " . $user->getName() . " " . Statistics::UNITS["balance"] . $args[1]);
				return true;
			}
		});
		return false;
