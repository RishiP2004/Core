...
class GiveCoins extends PluginCommand {
    ...
    public function __construct(Core $core) {
        parent::__construct("givecoins", $core);
		...
		$this->core->getStats()->getCoreUser($args[0], function($user) use ($sender, $args) {
			...
			if($user->getCoins() + $args[1] > Statistics::MAXIMUMS["coins"]) {
				...
				if($player instanceof CorePlayer) {
					$player->sendMessage($this->core->getPrefix() . $sender->getName() . " Gave you " . Statistics::UNITS["coins"] . $args[1]);
				}
				$sender->sendMessage($this->core->getPrefix() . "Gave " . $user->getName() . " " . Statistics::UNITS["coins"] . $args[1]);
				return true;
			}
        });
		return false;
