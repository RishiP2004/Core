...
class AddPlayerPermissionCommand extends PluginCommand {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool {
        ...
        if($user->hasDatabasedPermission($args[1])) {
            $sender->sendMessage($this->core->getPrefix() . $user->getUsername() . " already has the Permission " . $args[1]);
            return false;
        } else {
            ...
            $sender->sendMessage($this->core->getPrefix() . "Added the Permission " . $args[1] . " to " . $user->getUsername());
            return true;
        }
    }
}